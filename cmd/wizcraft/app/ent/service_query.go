...
func (sq *ServiceQuery) QueryDatabases() *DatabaseQuery {
	query := (&DatabaseClient{config: sq.config}).Query()
...
func (sq *ServiceQuery) QueryApispec() *APISpecQuery {
	query := (&APISpecClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServiceQuery) WithDatabases(opts ...func(*DatabaseQuery)) *ServiceQuery {
	query := (&DatabaseClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withDatabases = query
	return sq
}
...
func (sq *ServiceQuery) WithApispec(opts ...func(*APISpecQuery)) *ServiceQuery {
	query := (&APISpecClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	...
	if query := sq.withDatabases; query != nil {
		if err := sq.loadDatabases(ctx, query, nodes,
			func(n *Service) { n.Edges.Databases = []*Database{} },
			func(n *Service, e *Database) { n.Edges.Databases = append(n.Edges.Databases, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *ServiceQuery) loadDatabases(ctx context.Context, query *DatabaseQuery, nodes []*Service, init func(*Service), assign func(*Service, *Database)) error {
	...
	query.Where(predicate.Database(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.DatabasesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.service_databases
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_databases" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_databases" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *ServiceQuery) loadApispec(ctx context.Context, query *APISpecQuery, nodes []*Service, init func(*Service), assign func(*Service, *APISpec)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Service)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.APISpec(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.ApispecColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.service_apispec
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_apispec" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
